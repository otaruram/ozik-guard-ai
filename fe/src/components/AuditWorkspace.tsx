...
export function AuditWorkspace({ 
  ...
  const handleRegister = () => {
    ...
    request.onsuccess = (e: any) => {
      ...
      tx.objectStore("auditState").put({ file, fileName, fileSize, result: null, status: "configure", pageScope }, "savedAudit");
      tx.oncomplete = () => {
        navigate({ to: "/auth" });
      };
    };
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {result?.reviewStatus && (
            <div className={`px-8 py-4 border-b-4 flex items-start gap-3 ${
              result.reviewStatus === 'APPROVED' ? 'bg-emerald-50 border-emerald-400 text-emerald-950' :
              ...
              <div className="mt-0.5">
                {result.reviewStatus === 'APPROVED' && <CheckCircle2 className="h-5 w-5 text-emerald-600" />}
              ...
              <div className="flex-1">
                <h4 className="font-black uppercase text-sm">
                  {result.reviewStatus === 'APPROVED' ? 'Disetujui oleh Pakar' :
                   ...
                   result.reviewStatus === 'NEEDS_REVISION' ? 'Perlu Revisi (Tinjauan Pakar)' :
